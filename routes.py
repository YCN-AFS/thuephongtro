...
def book_property(property_id):
    ...
    if not property.is_available:
        flash('Bất động sản này hiện không còn trống.', 'danger')
        return redirect(url_for('property_details', property_id=property_id))
    
    # Make sure user is not booking their own property
    if property.owner_id == current_user.id:
        flash('Bạn không thể đặt phòng từ bất động sản của chính mình.', 'warning')
        return redirect(url_for('property_details', property_id=property_id))
    ...
    daily_price = property.price / 30  # Convert monthly price to daily
    total_price = daily_price * num_days
    
    # Check if there are conflicting bookings for the same property and dates
    ...
    if conflicting_bookings > 0:
        flash('Bất động sản này đã được đặt trong khoảng thời gian bạn chọn.', 'danger')
        return redirect(url_for('property_details', property_id=property_id))
    ...
    try:
        ...
        # Send a message to the property owner about the new booking
        message_text = f"""
        Xin chào,
        
        Tôi đã đặt phòng cho bất động sản của bạn: {property.title}
        
        Chi tiết đặt phòng:
        - Ngày nhận phòng: {check_in_date.strftime('%d/%m/%Y')}
        - Ngày trả phòng: {check_out_date.strftime('%d/%m/%Y')}
        - Số lượng khách: {num_guests}
        """
        ...
        # Update or create user chat entries
        update_user_chat(current_user.id, property.owner_id, new_message.id)
        
        db.session.commit()
        
        flash('Đặt phòng thành công! Bạn có thể xem chi tiết đặt phòng trong hồ sơ cá nhân của mình.', 'success')
        return redirect(url_for('my_bookings'))
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error booking property: {str(e)}")
        flash(f'Lỗi khi đặt phòng: {str(e)}', 'danger')
        return redirect(url_for('property_details', property_id=property_id))


# Helper function to update user chat entries
def update_user_chat(user_id, chat_with_id, message_id):
    # Update or create a user chat entry for the sender
    user_chat = UserChat.query.filter_by(
        user_id=user_id,
    ...
    if not user_chat:
        ...
        user_chat.chat_with_id = chat_with_id
        db.session.add(user_chat)
    
    # Update last message and timestamp
    user_chat.last_message_id = message_id
    user_chat.updated_at = datetime.now()
    
    # Also create/update the other side of the conversation
    receiver_chat = UserChat.query.filter_by(
        user_id=chat_with_id,
        chat_with_id=user_id
    ).first()
    
    if not receiver_chat:
        receiver_chat = UserChat()
        receiver_chat.user_id = chat_with_id
        receiver_chat.chat_with_id = user_id
        db.session.add(receiver_chat)
    
    # Update receiver's chat
    receiver_chat.last_message_id = message_id
    receiver_chat.unread_count = receiver_chat.unread_count + 1
    receiver_chat.updated_at = datetime.now()
...
# View bookings for landlord's properties
@app.route('/property-bookings')
@require_login
def property_bookings():
    ...
    return render_template('property_bookings.html', bookings=bookings)
...
def update_booking_status(booking_id):
    ...
    if property.owner_id != current_user.id:
        flash('Bạn không có quyền cập nhật trạng thái đặt phòng này.', 'danger')
        return redirect(url_for('property_bookings'))
    
    new_status = request.form.get('status')
    if new_status not in ['pending', 'confirmed', 'cancelled', 'completed']:
        flash('Trạng thái đặt phòng không hợp lệ.', 'danger')
        return redirect(url_for('property_bookings'))
    ...
    try:
        ...
        # Notify the renter about the status change
        message_text = f"""
        Trạng thái đặt phòng của bạn cho {property.title} đã được cập nhật thành: {new_status.upper()}.
        
        Chi tiết đặt phòng:
        - Ngày nhận phòng: {booking.check_in_date.strftime('%d/%m/%Y')}
        - Ngày trả phòng: {booking.check_out_date.strftime('%d/%m/%Y')}
        - Tổng tiền: {"{:,.0f}".format(booking.total_price)} VND
        """
        
        # Create a message to notify the renter
        new_message = Message()
        new_message.sender_id = current_user.id
        new_message.receiver_id = booking.renter_id
        new_message.property_id = booking.property_id
        new_message.message = message_text
        new_message.is_read = False
        
        db.session.add(new_message)
        
        # Update user chat entries
        update_user_chat(current_user.id, booking.renter_id, new_message.id)
        
        db.session.commit()
        
        flash(f'Trạng thái đặt phòng đã được cập nhật thành "{new_status}".', 'success')
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error updating booking status: {str(e)}")
        flash(f'Lỗi khi cập nhật trạng thái đặt phòng: {str(e)}', 'danger')
...
def cancel_booking(booking_id):
    ...
    if booking.renter_id != current_user.id:
        flash('Bạn không có quyền hủy đặt phòng này.', 'danger')
        return redirect(url_for('my_bookings'))
    
    # Check if booking can be cancelled (not already cancelled or completed)
    if booking.status in ['cancelled', 'completed']:
        flash('Không thể hủy đặt phòng đã hoàn tất hoặc đã hủy.', 'danger')
        return redirect(url_for('my_bookings'))
    ...
    try:
        ...
        # Notify the property owner about the cancellation
        message_text = f"""
        Đặt phòng cho {property.title} đã bị hủy bởi người thuê.
        ...
        flash(f'Lỗi khi hủy đặt phòng: {str(e)}', 'danger')
